...
window_size = descriptor_window_image_width / 4;

for i = 1 : size(x, 1)
    xrange = (x(i) - 2 * window_size) : (x(i) + 2 * window_size - 1);
    yrange = (y(i) - 2 * window_size) : (y(i) + 2 * window_size - 1);
    frame = image(yrange, xrange);

    [frame_mag, frame_dir] = imgradient(frame);
